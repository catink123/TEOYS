...
init python:
    ...
    splash_message_default = _("Авторы мода: Catink123 и SmA.")
...
label splashscreen:
    ...
    if not firstrun: #renpy.loadable("10"):
        if persistent.first_run and not persistent.do_not_delete:
            ...
            scene black
            menu:
                "Был найден файл сохранения. Хотите удалить выши данные сохранения и начать заново?"
                "Да, удалить существующие данные":
                    "Удаление данных сохранения...{nw}"
                    python:
                        delete_all_saves()
                        renpy.loadsave.location.unlink_persistent()
                        renpy.persistent.should_save_persistent = False
                        renpy.utter_restart()
                "Нет, продолжить с последнего момента":
                    pass
    ...
    if not persistent.first_run:
        ...
        "[config.name] это фан мод для игры Доки Доки Литературный Клуб, который не касается Team Salvato."
        "По идее требуется полное прохождение официальной игры."
        "Файлы игры Доки Доки Литературный Клуб нужны для этого мода и могут быть скачаны бесплатно по этой ссылке: http://ddlc.moe"
        ...
        menu:
            "Играя в мод [config.name] вы соглашаетесь, что вы прошли Доки Доки Литературный Клуб и принимаете любые содержащие спойлеры."
    ...
    $ config.main_menu_music = audio.mm_music
    $ renpy.music.play(config.main_menu_music)
...
label after_load:
    ...
    if anticheat != persistent.anticheat:
        stop music
        scene black
        "Файл сохранения не может быть открыт."
        "Вы пытаетесь читерить?"
